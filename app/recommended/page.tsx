...
interface RecommendedMangaType {
  id: string;
  title: string;
  genre: string;
  chapter: string;
...
export default function RecommendedPage() {
  ...
  const [mangaList, setMangaList] = useState<RecommendedMangaType[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchRecommendedManga = async () => {
      try {
        // placement нь recommended буюу санал болгох мангуудыг шүүнэ
        ...
        console.error("Санал болгох манга уншихад алдаа гарлаа:", error);
        setLoading(false);
      }
    };
  ...
  return (
    <main className="min-h-screen bg-[#0B0F14] text-white">
      <div className="mx-auto max-w-7xl px-6 md:px-8 py-10">
        ...
        {/* ТАНСАГ 6 ЦУВАА КАВЕР КАРТ БҮТЭЦ */}
        <div className="grid gap-4 md:gap-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          {loading ? (
            ...
            Array.from({ length: 6 }).map((_, idx) => (
              <div key={`recommended-page-skeleton-${idx}`} className="animate-pulse border border-[#222933] bg-[#141922]/40 rounded-2xl md:rounded-3xl overflow-hidden flex flex-col h-[340px]">
            ...
            <div className="col-span-full py-16 text-center text-gray-500 text-sm font-medium">
              Одоогоор санал болгосон манга байхгүй байна.
            </div>
          ) : (
            mangaList.map((item) => (
              <div
                ...
                onClick={() => {
                  if (typeof window !== "undefined") {
                    window.history.replaceState({ usr: { from: "/recommended" } }, "");
                  }
